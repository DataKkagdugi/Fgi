...
dag = DAG(
    dag_id = 'project3_Gsheet_to_Redshift',
    start_date = datetime(2024,11,20), # 날짜가 미래인 경우 실행이 안됨
    schedule = '0 9 * * *',  # 적당히 조절
    max_active_runs = 1,
    max_active_tasks = 2,
    catchup = False,
...
sheets = [
    {
        "url": "https://docs.google.com/spreadsheets/d/1o_O7Qkc_JNH4aJ8OnrvrgagB0JH_Yef6k8TvoMQjL9U/edit?usp=sharing",
        "tab": "fgi",
        "schema": "raw_data",
...
for sheet in sheets:
    ...
    run_copy_sql = S3ToRedshiftOperator(
        ...
        method = 'REPLACE',
        redshift_conn_id = "project_redshift_db",
